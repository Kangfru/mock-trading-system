...
class OrderConsumer(
    ...
    fun consumeOrder(order: Order) {
        when (order.action) {
            OrderAction.NEW -> logger.info(
                "Received [NEW]: orderNumber={}, [{}] {} - {}주 @ {}원",
                order.orderNumber, order.orderType, order.stockCode, order.quantity, order.price
            )
            OrderAction.CANCEL -> logger.info(
                "Received [CANCEL]: orderNumber={}, originalOrderNumber={}",
                order.orderNumber, order.originalOrderNumber
            )
            OrderAction.MODIFY -> logger.info(
                "Received [MODIFY]: orderNumber={}, originalOrderNumber={}, newQty={}, newPrice={}",
                order.orderNumber, order.originalOrderNumber, order.quantity, order.price
            )
        }
