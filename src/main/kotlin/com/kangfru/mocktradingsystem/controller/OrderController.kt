...
class OrderController(
    ...
    fun createOrder(@RequestBody request: CreateOrderRequest): Order {
        val price = when (request.priceType) {
            PriceType.MARKET -> {
                // 시장가 주문: 현재 시세 조회
                val quote = stooqClient.getQuote(request.stockCode)
    ...
    @PostMapping("/cancel")
    fun cancelOrder(@RequestBody request: CancelOrderRequest): Order {
    ...
    @PostMapping("/modify")
    fun modifyOrder(@RequestBody request: ModifyOrderRequest): Order {
        val modifyOrder = Order(
            orderNumber = snowflakeIdGenerator.nextId(),
    ...
    @PostMapping("/bulk")
    fun createBulkOrders(@RequestParam(defaultValue = "100") count: Int): Map<String, Any> {
    ...
    fun createMixedBulkOrders(
        ...
        val cancelOrders = ordersToCancel.map { original ->
            Order(
                orderNumber = snowflakeIdGenerator.nextId(),
                stockCode = "",
        ...
        val ordersToModify = newOrders.filterNot { ordersToCancel.contains(it) }.shuffled().take(modifyCount.coerceAtMost(newCount - cancelCount))
        val modifyOrders = ordersToModify.map { original ->
            Order(
                orderNumber = snowflakeIdGenerator.nextId(),
                stockCode = "",
                orderType = OrderType.BUY,
    ...
    fun getStats(): Map<String, Any> {
        return executionService.getStats()
    }
