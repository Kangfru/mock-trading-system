...
class UserServiceTest {
    ...
    @Test
    fun `should throw exception when creating user with blank username`() = runTest {
    ...
    @Test
    fun `should throw exception when creating user with blank email`() = runTest {
        assertThrows<IllegalArgumentException> {
    ...
    @Test
    fun `should throw exception when creating user with duplicate email`() = runTest {
    ...
    @Test
    fun `should get user by seq`() = runTest {
    ...
    @Test
    fun `should throw exception when getting non-existent user`() = runTest {
    ...
    @Test
    fun `should throw exception when getting user by non-existent email`() = runTest {
        assertThrows<NoSuchElementException> {
    ...
    @Test
    fun `should update user username`() = runTest {
        val created = userService.createUser("testuser", "test@example.com")
    ...
    @Test
    fun `should update user email`() = runTest {
        val created = userService.createUser("testuser", "test@example.com")
    ...
    @Test
    fun `should throw exception when updating email to existing one`() = runTest {
        userService.createUser("user1", "user1@example.com")
    ...
    @Test
    fun `should add account to user`() = runTest {
        val user = userService.createUser("testuser", "test@example.com")
    ...
    @Test
    fun `should throw exception when adding duplicate account`() = runTest {
        val user = userService.createUser("testuser", "test@example.com")
    ...
    @Test
    fun `should remove account from user`() = runTest {
        val user = userService.createUser("testuser", "test@example.com")
        userService.addAccountToUser(user.seq, "ACC-001")
    ...
    @Test
    fun `should throw exception when removing non-existent account`() = runTest {
        val user = userService.createUser("testuser", "test@example.com")
    ...
    @Test
    fun `should get user accounts`() = runTest {
        val user = userService.createUser("testuser", "test@example.com")
        userService.addAccountToUser(user.seq, "ACC-001")
    ...
    @Test
    fun `should update updatedAt timestamp when updating user`() = runTest {
        val user = userService.createUser("testuser", "test@example.com")
    ...
    inner class ConcurrencyTests {
        ...
        @Test
        fun `should handle concurrent account additions correctly`() = runTest {
            // Given
            val user = userService.createUser("testuser", "test@example.com")
            val accountCount = 10

            // When: add 10 accounts concurrently
            val jobs = List(accountCount) { index ->
                async {
        ...
        @Test
        fun `should handle concurrent account removals correctly`() = runTest {
            // Given
            val user = userService.createUser("testuser", "test@example.com")
            val accountCount = 10
        ...
        @Test
        fun `should handle concurrent updates correctly`() = runTest {
            // Given
            val user = userService.createUser("testuser", "test@example.com")
            val updateCount = 10
        ...
        @Test
        fun `should handle concurrent account additions and removals correctly`() = runTest {
            // Given
            val user = userService.createUser("testuser", "test@example.com")
            ...
            val additions = List(operationsPerType) { index ->
                async {
                    userService.addAccountToUser(user.seq, "ACC-NEW-${index.toString().padStart(3, '0')}")
                }
            }
            val removals = List(operationsPerType) { index ->
                async {
